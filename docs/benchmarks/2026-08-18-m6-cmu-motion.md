...
malformed, no source hash drift or undeclared contact mismatch occurred, and
the derived soft baselines are 21 mm foot slide, 3 mm trajectory deviation,
60,005 microradians adjacent turn discontinuity, and 0 rejected frames per
million. Target-skeleton retargeting is not executed by this lane and is
reported as not applicable rather than as zero failures.
...
- Repository base before Fix Round 1: `fdcc236`
...
the converted database stayed under `/tmp/blender-crowd-m6-cmu-fix1`; only source
...
rotations by pre-multiplication for column vectors; evaluated world-space root
and foot transforms; and retained source frames 1, 5, 9, and so on for
deterministic 120 Hz to 30 Hz conversion. Root motion follows the rotational
channel sequence in `root_order`, independently of the static root-axis order.
...
measures undeclared contact, hash drift, and joint-limit violations. Runtime
root transitions and cache mutation are not executed here, so those two hard
requirements are explicitly not applicable to this report rather than recorded
as observed zeros.
...
cache immutability, performance, or compatibility with arbitrary terrain. The
terrain-related evidence is limited to
