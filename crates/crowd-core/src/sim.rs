...
use crate::motion::MotionMatcher;
use crate::nav::{PortalId, TileGraph};
use crate::phases::animate::{animate, animate_scheduled, apply_motion_matches, AnimateConfig};
...
pub struct Simulation {
    ...
    motion_matcher: Option<MotionMatcher>,
}
...
impl Simulation {
    ...
    /// Attach an optional trajectory matcher for promoted S0 agents. The
    /// deterministic clip-state baseline remains authoritative for all other
    /// tiers and for every matcher failure.
    pub fn set_motion_matcher(&mut self, matcher: MotionMatcher) {
        self.motion_matcher = Some(matcher);
    }
    ...
    pub fn authorable_activity_status(
        &self,
        agent_id: u64,
        resource_id: &str,
    ) -> Option<crate::activity::ReservationStatusV1> {
        self.authorable_behavior
            .as_ref()?
            .activity_status(agent_id, resource_id)
    }

    pub fn authorable_formation_report(
        &self,
        formation_id: &str,
    ) -> Option<crate::formation::FormationReportV1> {
        self.authorable_behavior
            .as_ref()?
            .formation_report(formation_id)
    }

    pub fn enqueue_authorable_interaction(
        &mut self,
        request: crate::interaction::InteractionRequestV1,
    ) -> Result<(), String> {
        self.authorable_behavior
            .as_mut()
            .ok_or_else(|| "authorable behavior controller is not enabled".to_owned())?
            .enqueue_interaction(request)
    }

    pub fn promote_authorable_interaction_next(&mut self) -> Option<String> {
        self.authorable_behavior
            .as_mut()?
            .promote_interaction_next()
    }

    pub fn authorable_interaction_status(
        &self,
        request_id: &str,
    ) -> Option<crate::interaction::InteractionGroupStatusV1> {
        self.authorable_behavior
            .as_ref()?
            .interaction_status(request_id)
    }
    ...
    pub fn step(&mut self) {
        ...
        if let Some(matcher) = &self.motion_matcher {
            apply_motion_matches(&mut self.world, matcher);
        }
        self.world.commit();
